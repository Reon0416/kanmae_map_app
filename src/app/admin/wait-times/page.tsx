...
export default async function AdminWaitTimesPage({ searchParams }: { searchParams?: Promise<{ saved?: string }> }) {
  const stores = await getStores();
  const savedStoreId = (await searchParams)?.saved;
    ...
    >
      <section className="border border-slate-200 bg-white">
        ...
        <div className="divide-y divide-slate-200">
          {stores.map((store) => {
            const saved = savedStoreId === store.id;

            return (
              <form
                action={updateWaitTimeAction}
                key={store.id}
              ...
              >
                ...
                <WaitTimeSubmitButton saved={saved} />
              </form>
            );
          })}
        </div>
