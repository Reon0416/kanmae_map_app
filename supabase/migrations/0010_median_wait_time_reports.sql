...
create index if not exists crowd_reports_store_created_at_idx
  on public.crowd_reports (store_id, created_at desc);

create index if not exists crowd_reports_store_user_created_at_idx
  on public.crowd_reports (store_id, created_by, created_at desc);
...
begin
  ...
  with latest_user_reports as (
    select distinct on (coalesce(created_by::text, id::text))
      wait_time,
      case wait_time
        when 'no_wait' then 0
    ...
    order by coalesce(created_by::text, id::text), created_at desc
  ),
  ranked_reports as (
    select
      wait_time,
      row_number() over (order by wait_score, wait_time::text) as row_number,
